const global = {
    colors: []
}

const setup = () => {
    let colorDemos = document.getElementsByClassName("colorDemo");
    let sliders = document.getElementsByClassName("slider");
    document.getElementById("save").addEventListener("click", saveColor);

    for (let i = 0; i < sliders.length; i++) {
        sliders[i].addEventListener("change", update);
        sliders[i].addEventListener("input", update);
    }

    // maak het blokje rood
    colorDemos[0].style.backgroundColor = "rgb(" + 50 + "," + 50 + "," + 50 + ")";

    loadColors();
    loadSliders();


}

const loadSliders = () => {
    let sliders = document.getElementsByClassName("slider");
    let colorDemos = document.getElementsByClassName("colorDemo");

    let red = localStorage.getItem("r");
    let green = localStorage.getItem("g");
    let blue = localStorage.getItem("b");

    let r = document.getElementById("r");
    let g = document.getElementById("g");
    let b = document.getElementById("b");

    if (red != null) {
        r.innerHTML = "R: " + red;
        sliders[0].value = red;
    }
    if (green != null) {
        g.innerHTML = "G: " + green;
        sliders[1].value = green;
    }
    if (blue != null) {
        b.innerHTML = "B: " + blue;
        sliders[2].value = blue;
    }

    colorDemos[0].style.backgroundColor = "rgb(" + red + "," + green + "," + blue + ")";
}

const loadColors = () => {
    global.colors = localStorage.getItem("colors");
    if (global.colors !== null) {
        global.colors = JSON.parse(global.colors);
        let colorList = document.getElementById("colorList");
        for (let i = 0; i < global.colors.length; i++) {
            colorList.appendChild(createDivJSON(global.colors[i]));
        }
    } else {
        global.colors = [];
    }

}

const createDivJSON = (data) => {
    let div = document.createElement("div");
    div.className = data.className;
    div.addEventListener("click", chooseColor);
    div.style.backgroundColor = data.divBackgroundColor;
    //add custom id to div
    div.id = data.id;
    // add button
    let button = document.createElement("button");
    button.className = data.buttonClassName;
    button.innerHTML = data.buttonInnerHTML;
    button.addEventListener("click", removeColor);
    div.appendChild(button);

    return div;
}

const update = () => {
    let sliders = document.getElementsByClassName("slider");
    let colorDemos = document.getElementsByClassName("colorDemo");
    let r = document.getElementById("r");
    let g = document.getElementById("g");
    let b = document.getElementById("b");


    r.innerHTML = "R: " + sliders[0].value;
    g.innerHTML = "G: " + sliders[1].value;
    b.innerHTML = "B: " + sliders[2].value;

    colorDemos[0].style.backgroundColor = "rgb(" + sliders[0].value + "," + sliders[1].value + "," + sliders[2].value + ")";

    localStorage.setItem("r", sliders[0].value);
    localStorage.setItem("g", sliders[1].value);
    localStorage.setItem("b", sliders[2].value);

}

const saveColor = () => {

    let div = createDiv()
    colorList.appendChild(div);

    var colordiv = {
        className: "colorDiv",
        divBackgroundColor: div.style.backgroundColor,
        id: div.id,
        buttonClassName: "colorButton",
        buttonInnerHTML: "X"

    }

    if (!isColorPresent(div)) {
        global.colors.push(colordiv);
        localStorage.setItem("colors", JSON.stringify(global.colors));
    }
}

const createDiv = () => {
    let sliders = document.getElementsByClassName("slider");
    let color = "rgb(" + sliders[0].value + "," + sliders[1].value + "," + sliders[2].value + ")";

    let colorList = document.getElementById("colorList");
    // setting up div
    let div = document.createElement("div");
    div.className = "colorDiv";
    div.addEventListener("click", chooseColor);
    div.style.backgroundColor = color;
    //add custom id to div
    div.id = "r" + sliders[0].value + "g" + sliders[1].value + "b" + sliders[2].value;
    // add button
    let button = document.createElement("button");
    button.className = "colorButton";
    button.innerHTML = "X";
    button.addEventListener("click", removeColor);
    div.appendChild(button);

    return div;
}

const removeColor = (event) => {
    let colorList = document.getElementById("colorList");

    for (let i = 0; i < global.colors.length; i++) {
        if (event.target.parentNode.id === global.colors[i].id) {
            colorList.removeChild(event.target.parentElement);
            global.colors.splice(i, 1);
            localStorage.setItem("colors", JSON.stringify(global.colors));
        }
    }
}

const chooseColor = (event) => {

    let rgbvalues = event.target.style.backgroundColor.substring(4, event.target.style.backgroundColor.length - 1).split(",");

    let sliders = document.getElementsByClassName("slider");
    if (rgbvalues.length === 3) {
        sliders[0].value = rgbvalues[0].trim();
        sliders[1].value = rgbvalues[1].trim();
        sliders[2].value = rgbvalues[2].trim();

        document.getElementById("r").innerHTML = "R: " + sliders[0].value;
        document.getElementById("g").innerHTML = "G: " + sliders[1].value;
        document.getElementById("b").innerHTML = "B: " + sliders[2].value;

        document.getElementsByClassName("colorDemo")[0].style.backgroundColor
            = "rgb(" + sliders[0].value + "," + sliders[1].value + "," + sliders[2].value + ")";

        localStorage.setItem("r", sliders[0].value);
        localStorage.setItem("g", sliders[1].value);
        localStorage.setItem("b", sliders[2].value);
    }
}

const isColorPresent = (color) => {
    return global.colors.includes(color);
}


window.addEventListener("load", setup);